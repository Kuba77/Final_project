...
describe("Testing of ProductStore component", () => {
    test("render ProductStore quantity", () => {
        render(
            <ProductStore product={product.quantity} />
        );
        screen.debug();
    });

    test("ProductStore have class attribute", () => {
      const{getByTestId} = render(
         <p data-testid="test" className="test">{product.store}</p>
       );
