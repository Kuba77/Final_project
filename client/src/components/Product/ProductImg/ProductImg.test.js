...
describe("Testing of ProductImg component", () => {
    test("render ProductImg", () => {
        render(
            <ProductImg src={product.imageUrls[0]} alt={product.name}/>
        );
        screen.debug();
    });

    it("ProductImg contain alt text", () => {
      render(
        <ProductImg src={product.imageUrls[0]} alt={product.name}/>
      );
      expect(screen.getByAltText(/gantz/i)).toBeInTheDocument()
    });
});